# Управление пакетами RPM и DNF

**Языки:** [English](README.md) · [简体中文](README_zh.md) · [Español](README_es.md) · [Français](README_fr.md) · [Deutsch](README_de.md) · [日本語](README_ja.md) · [Русский](README_ru.md) · [한국어](README_ko.md) · [Português](README_pt.md)

<p align="center">
  <a href="https://labex.io/ru/courses/rpm-and-dnf-package-management">
    <img src="https://course-cover.labex.io/rpm-and-dnf-package-management.png?lang=ru" alt="Управление пакетами RPM и DNF">
  </a>
</p>

Изучите управление пакетами RPM и DNF. Этот модуль охватывает запросы пакетов RPM, установку и удаление с помощью RPM, управление пакетами с помощью DNF, историю и отмену DNF, создание локального репозитория. Вы освоите эти основные навыки Linux с помощью практических лабораторных работ и реальных задач.

[Начать курс на LabEx](https://labex.io/ru/courses/rpm-and-dnf-package-management)

## Упражнения

|   Индекс | Название                                           | Сложность   | Практика                                                                                                                                                |
|----------|----------------------------------------------------|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------|
|        1 | Запросы к RPM-пакетам                              | Начинающий  | <a target='_blank' href='https://labex.io/ru/labs/rpm-package-queries-636552?course=rpm-and-dnf-package-management'>Начать лабораторию</a>              |
|        2 | Установка и удаление пакетов с помощью RPM         | Начинающий  | <a target='_blank' href='https://labex.io/ru/labs/installing-and-removing-with-rpm-636551?course=rpm-and-dnf-package-management'>Начать лабораторию</a> |
|        3 | История транзакций и отмена действий в DNF         | Начинающий  | <a target='_blank' href='https://labex.io/ru/labs/dnf-history-and-undo-636550?course=rpm-and-dnf-package-management'>Начать лабораторию</a>             |
|        4 | Создание локального репозитория                    | Начинающий  | <a target='_blank' href='https://labex.io/ru/labs/creating-a-local-repository-636549?course=rpm-and-dnf-package-management'>Начать лабораторию</a>      |
|        5 | Корпоративный репозиторий программного обеспечения | Средний     | <a target='_blank' href='https://labex.io/ru/labs/enterprise-software-repo-636737?course=rpm-and-dnf-package-management'>Начать испытание</a>           |

## About LabEx

<div align="left"><p><a href="https://labex.io"><strong>LabEx</strong></a> is a <strong>hands-on learning platform for beginners</strong>.</p><p>Explore <a href="https://labex.io/learn/linux"><strong>Linux</strong></a>, <a href="https://labex.io/learn/devops"><strong>DevOps</strong></a>, <a href="https://labex.io/learn/cybersecurity"><strong>Cybersecurity</strong></a>, and <strong>more</strong> — all directly in your browser.</p><p>Learn step by step through <strong>interactive labs</strong>, <strong>guided exercises</strong>, and <strong>real-world projects</strong>. 🌱<br />No setup, no stress — just practice and grow your skills by doing.</p><br /><p><a href="https://apps.apple.com/app/id6765840991"><img src="https://developer.apple.com/assets/elements/badges/download-on-the-app-store.svg" alt="Download on the App Store" height="54" /></a>&nbsp;<a href="https://apps.apple.com/app/id6765840991"><img src="https://developer.apple.com/app-store/marketing/guidelines/images/badge-download-on-the-mac-app-store.svg" alt="Download on the Mac App Store" height="52" /></a></p><br /><p>📖 Need help? Visit our <a href="https://support.labex.io/">Help Center</a> or email info@labex.io</p></div>

